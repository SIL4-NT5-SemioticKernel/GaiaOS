...
class LedIndicator(tk.Canvas):
    ...
    # Back-compat: some callers use set_state()
    def set_state(self, on: bool, on_color=None, off_color=None):
        return self.set_on(on, on_color=on_color, off_color=off_color)
...
class GaiaConfigUI(tk.Tk):
    def __init__(self):
        ...
        # Dashboard history sources:
        # We prefer plotting from the same history-window files used by "Under The Hood" (Skelly_Panels.ssv),
        # falling back to in-GUI sampled status values only when no suitable file source is found.
        self._dashboard_history_sources = {}   # (kind, idx) -> {"type": "panel"/"file"/"status", ...}
        self._dashboard_file_cache = {}        # path -> {"mtime": float, "parsed": dict}
        self._dashboard_panel_cache = {}       # panel_id -> {"mt_sig": tuple, "data": dict}
        self._dashboard_last_update_text = ""  # rendered in dashboard header
    ...
    def _build_summary_dashboard(self):
        ...
        # Shows when the dashboard last refreshed (helps confirm updates even on flat signals)
        self.dash_update_var = tk.StringVar(value="")
        ttk.Label(hdr, textvariable=self.dash_update_var).pack(side=tk.LEFT, padx=(10, 0))
    ...
    def _dashboard_clear_history(self):
        # Note: if plots are sourced from engine history-window files, this does not delete/alter those files.
        # It clears only GUI-side caches/buffers.
        self.dashboard_histories = {}
        self._dashboard_sample_counter = 0
        self._dashboard_file_cache = {}
    ...
    def _dashboard_resolve_history_sources(self, slots):
        """Resolve per-row history sources.

        Priority:
          1) A matching panel in Skelly_Panels.ssv (same files/logic as Under The Hood)
          2) A best-guess history/log file under System_State_Files/
          3) Fallback: in-GUI sampled status values (self.dashboard_histories)
        """
        sources = {}
        panel_defs = getattr(self, "panel_defs", None) or []

        for kind, idx, ent in slots:
            key = (kind, idx)
            channel = f"{kind}{idx}" if isinstance(idx, int) and idx >= 0 else None
            src = None

            # 1) Prefer the same sources used by Under The Hood via Skelly_Panels.ssv
            if channel and panel_defs:
                pd = self._dashboard_pick_panel_for_channel(channel, panel_defs)
                if pd is not None:
                    series_idx = None
                    # If this panel is an io_log/matrix with multiple series, we need an index.
                    if getattr(pd, "kind", "") == "io_log":
                        series_idx = idx
                    elif getattr(pd, "kind", "") == "matrix_rows":
                        series_idx = idx
                    src = {"type": "panel", "panel": pd, "series_idx": series_idx}

            # 2) Best-effort file search under System_State_Files/
            if src is None and channel:
                fp = self._dashboard_find_history_file_for_channel(channel)
                if fp:
                    src = {"type": "file", "path": fp, "series_idx": idx}

            # 3) Status-sampled fallback
            if src is None:
                src = {"type": "status"}

            sources[key] = src

        self._dashboard_history_sources = sources

    def _dashboard_pick_panel_for_channel(self, channel: str, panel_defs):
        """Pick the best matching PanelDef for a channel token like 'A0' or 'E2'."""
        token = re.escape(channel)
        # token boundary: avoid matching A0 inside A01
        patt = re.compile(rf"(?<![A-Za-z0-9]){token}(?![A-Za-z0-9])")

        best = None
        best_score = -10**9
        for pd in panel_defs:
            hay = f"{getattr(pd, 'panel_id', '')} {getattr(pd, 'label', '')}"
            if not patt.search(hay):
                continue

            label = getattr(pd, "label", "") or ""
            panel_id = getattr(pd, "panel_id", "") or ""
            kind = getattr(pd, "kind", "") or ""
            ...
            if label == channel:
                ...
                score += 80
            elif panel_id == channel:
                score += 70
            elif panel_id.startswith(channel):
                score += 60
            else:
                score += 40

            # Prefer kinds that naturally represent a single-channel history window
            score += {"xy": 35, "xy_multi": 25, "matrix_rows": 20, "io_log": 10}.get(kind, 0)

            # Prefer single-file sources (less ambiguous)
            try:
                if len(getattr(pd, "files", []) or []) == 1:
                    score += 5
            except Exception:
                pass
    ...
    def _dashboard_find_history_file_for_channel(self, channel: str):
        """Heuristic fallback: find a history/log file for a channel under System_State_Files/."""
        base = SYSTEM_STATE_DIR
        if not os.path.isdir(base):
            return None

        import glob
        # Keep the search small and biased toward likely names.
        pats = [
            os.path.join(base, "**", f"{channel}*.*"),
            os.path.join(base, "**", f"*{channel}*.*"),
        ]
        candidates = []
        for p in pats:
        ...
        # Filter to SSV-like files first (but don't hard-fail if they aren't .ssv)
        filtered = []
        for c in candidates:
            if not os.path.isfile(c):
                continue
            bn = os.path.basename(c).lower()
            if bn in ("status.ssv", "control_panel.ssv", "update_flag.ssv"):
                continue
            if bn.endswith(".ssv") or bn.endswith(".cfg") or bn.endswith(".txt"):
                filtered.append(c)
        ...
        def _score(path):
            ...
            score = 0
            if bn.startswith(channel.lower()):
                score += 20
            if "hist" in bn or "history" in bn:
                score += 30
            if "window" in bn:
                score += 25
            if "log" in bn:
                score += 15
            # Prefer shallower paths (usually the canonical ones)
            score -= path.count(os.sep)
            return score

        filtered.sort(key=lambda p: _score(p), reverse=True)
        return filtered[0]

    def _dashboard_load_history_series(self, kind, idx, max_points):
        """Return (x, y) for the row history, preferring panel/file sources."""
        src = (self._dashboard_history_sources or {}).get((kind, idx)) or {"type": "status"}

        if src.get("type") == "panel":
            pd = src.get("panel")
            if pd is None:
                return [], []
            # Cache panel computations by file mtime signature to avoid re-reading large logs every second
            pid = getattr(pd, 'panel_id', None) or str(id(pd))
            try:
                mt_sig = tuple(os.path.getmtime(p) if os.path.exists(p) else None for p in (getattr(pd, 'files', []) or []))
            except Exception:
                mt_sig = None

            cached = None
            try:
                cached = (self._dashboard_panel_cache or {}).get(pid)
            except Exception:
                cached = None

            if cached and cached.get('mt_sig') == mt_sig and cached.get('data') is not None:
                data = cached.get('data')
            else:
                data = self._compute_panel_data(pd)
                try:
                    self._dashboard_panel_cache[pid] = {'mt_sig': mt_sig, 'data': data}
                except Exception:
                    pass
            ...
            # Choose a single series
            sidx = src.get("series_idx")
            y = None
            if isinstance(sidx, int) and 0 <= sidx < len(series):
                y = series[sidx]
            elif series:
                y = series[0]
            else:
        ...
        if src.get("type") == "file":
            path = src.get("path")
            sidx = src.get("series_idx")
            x, y = self._dashboard_load_series_from_file(path, max_points=max_points, series_idx=sidx)
            return x, y

        # status fallback: use in-GUI sampled history buffers
        h = (self.dashboard_histories or {}).get((kind, idx))
        if not h:
            return [], []
        return list(h.get("x", [])), list(h.get("y", []))

    def _dashboard_load_series_from_file(self, path: str, max_points: int = 180, series_idx: int | None = None):
        """Parse a numeric history/log file into (x, y).

        Supported common forms:
          - y-only: one float per line  -> x = [0..]
          - xy:     two floats per line -> (x, y)
          - io_log: tick v0 v1 v2 ...   -> choose v[series_idx]
        """
        ...
        cache = self._dashboard_file_cache.get(path)
        if cache and cache.get("mtime") == mtime and cache.get("parsed"):
            parsed = cache["parsed"]
        else:
            parsed = None
            # Try strict XY first (matches many Under-The-Hood series files)
            xs, ys = load_ssv_xy(path)
            if xs and ys:
                parsed = {"mode": "xy", "x": xs, "y": ys}
            else:
                # Generic numeric-row parser
                from collections import deque
                keep = max_points if (isinstance(max_points, int) and max_points > 0) else 2000
                rows = deque(maxlen=max(keep, 50))
                try:
                    with open(path, "r", encoding="utf-8") as f:
                        for line in f:
                            line = line.strip()
                ...
                if not rows:
                    ...
                    row_len = max(lens.items(), key=lambda kv: kv[1])[0]
                    ...
                    if not rows:
                        ...
                        y = [r[0] for r in rows]
                        x = list(range(len(y)))
                        parsed = {"mode": "y", "x": x, "y": y}
                    elif row_len == 2:
                        x = [r[0] for r in rows]
                        y = [r[1] for r in rows]
                        parsed = {"mode": "xy", "x": x, "y": y}
                    else:
                        # tick + N channels
                        ...
                        for r in rows:
                            for i in range(n):
                                series[i].append(r[i + 1])
                        parsed = {"mode": "io_log", "x": ticks, "series": series}

            self._dashboard_file_cache[path] = {"mtime": mtime, "parsed": parsed}
        ...
        if mode == "xy":
            ...
            col = 0
            if isinstance(series_idx, int) and 0 <= series_idx < len(series):
                col = series_idx
            y = list(series[col]) if series else []
        else:
            return [], []
        ...
        # Trim
        if isinstance(max_points, int) and max_points > 0 and len(x) > max_points:
            x = x[-max_points:]
            y = y[-max_points:]
    ...
    def _dashboard_ensure_rows(self, slots):
        ...
        # Resolve history sources whenever rows are rebuilt (keeps the dashboard tied to Under-The-Hood data sources)
        try:
            self._dashboard_resolve_history_sources(slots)
        except Exception:
            pass
    ...
    def _dashboard_refresh_from_status(self, engine, afferents, efferents):
        ...
        # stamp the dashboard refresh time so users can confirm updates even on flat signals
        try:
            import time
            if hasattr(self, "dash_update_var"):
                self.dash_update_var.set(time.strftime("| Dashboard: %H:%M:%S"))
        except Exception:
            pass

        # x-axis sample index for status-sampled fallback history buffers
        x_now = float(self._dashboard_sample_counter)
        self._dashboard_sample_counter += 1.0

        # desired points shown (0 = unlimited)
        try:
            max_points = int(self.dashboard_history_points_var.get())
        except Exception:
            max_points = 180
        if max_points < 0:
            max_points = 0
        ...
        def _hist_for(key):
            ...
            if h is None:
                from collections import deque
                h = {
                    "x": deque(maxlen=(max_points if max_points else None)),
                    "y": deque(maxlen=(max_points if max_points else None)),
                }
                self.dashboard_histories[key] = h
            else:
                # update maxlen if user changed it
                if max_points:
                    if h["x"].maxlen != max_points:
                        from collections import deque
                        h["x"] = deque(h["x"], maxlen=max_points)
                        h["y"] = deque(h["y"], maxlen=max_points)
                else:
        ...
        for kind, idx, ent in slots:
            ...
            # Pull current instantaneous value/goal from status.ssv (for LEDs/text)
            val = None
            goal = None
            if kind == "A":
            ...
            # Update GUI-side fallback buffers from instantaneous status values.
            # Even when file-based plotting is active, keeping these buffers updated gives a robust fallback.
            key = (kind, idx)
            h = _hist_for(key)
            ...
            # Pick the plotted history from the preferred source (panel/file), falling back to the GUI buffer
            try:
                x_list, y_list = self._dashboard_load_history_series(kind, idx, max_points)
            except Exception:
                x_list, y_list = [], []
            ...
            if isinstance(ent, dict):
                desc = ent.get("desc", "")
            idx_lbl = f"{kind}{idx}" if isinstance(idx, int) and idx >= 0 else f"{kind}?"
            title = f"{idx_lbl} — {desc}" if desc else idx_lbl

            # right-aligned current value text
            if isinstance(val, (int, float)):
                w["value_var"].set(_fmt(val))
            elif val is None:
                w["value_var"].set("")
            else:
                w["value_var"].set(str(val))

            # Goal indicators (afferents only)
            has_goal = isinstance(goal, (int, float))
            w["goal_led"].set_state(bool(has_goal))
            w["goal_val_var"].set(_fmt(goal) if has_goal else "")

            # Low/high logic: only meaningful if we have both val and goal
            low_on = False
            high_on = False
            if has_goal and isinstance(val, (int, float)):
                low_on = (val < goal)
                high_on = (val > goal)
            w["low_led"].set_state(low_on)
            w["high_led"].set_state(high_on)

            # Output state (efferents)
            out_on = False
            if kind == "E" and isinstance(val, (int, float)):
                out_on = (val != 0.0)
            w["out_led"].set_state(out_on)
            w["out_var"].set("ON" if out_on else "")
